...
def calc_code_bleu(refs, hyps, lang, params=(0.25, 0.25, 0.25, 0.25)):
    ...
    for ref_list in refs:
        cur_pre_ref = []
        for ref in ref_list:
            cur_pre_ref.append(ref.strip())
        references.append(cur_pre_ref)
    ...
    # keywords_path = os.path.join(keywords_dir_path, args.lang + ".txt")

    keywords_path = os.path.join(PACKAGE_DIR, "keywords", lang + ".txt")
    keywords = [
        x.strip() for x in open(keywords_path, "r", encoding="utf-8").readlines()
    ]
